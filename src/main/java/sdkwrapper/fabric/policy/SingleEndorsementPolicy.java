...
public class SingleEndorsementPolicy implements EndorsementPolicyIF
{
  ...
  public boolean isPolicyMet(Collection<ProposalResponse> responses)
  {
    ...
    try
    {
      ...
      {
        System.out.println( "Response Valid = " + response.isInvalid() + "; Response Message = " + response.getMessage() + "; Status = " + response.getStatus() + "; Action Status = " + response.getChaincodeActionResponseStatus() );
